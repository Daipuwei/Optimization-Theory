	...
	public:
		...
		void Init(){
			...
			for(int i = 0 ; i < n ; i++){
				string str;
				cin>>str;
				if(type == -1){				// 当目标函数有求最小值时，要进行模型转化，转换为最大值 
					if(str[0] == '-'){		//str1为负数时，这去掉符号 
						str = str.erase(0,1);
					}else if(str[0] != '0'){  //str1为正数时，加上符号 
						str = "-"+str;
					}
				}
				Fraction C;
